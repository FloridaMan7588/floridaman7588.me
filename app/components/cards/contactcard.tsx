...
export default function AboutCard({ content, contacts }) {
	const contactList = []
	for (const contact of contacts) {
		const listId = contacts.indexOf(contact)
		contactList.push(<li className='py-2 md:py-4' key={listId}><Link href={contact.link}><div className='text-lavender hoverPop105'>{'• ' + contact.title}</div></Link></li>)
	}
	return (
		<div className='bg-ctp-mantle rounded-[45px]'>
			<h1 className='text-ctp-text text-center text-4xl font-bold px-7 py-4'>Contact Me</h1>
